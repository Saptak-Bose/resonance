          ...
          {items.map((item) => (
            <SidebarMenuItem key={item.title}>
              <SidebarMenuButton
                ...
                tooltip={item.title}
                className="h-9 px-3 py-2 text-[16px] tracking-tight font-medium border border-transparent data-[active=true]:border-border data-[active=true]:shadow-[0px_1px_1px_0px_rgba(44,54,53,0.03),inset_0px_0px_0px_2px_white]"
              >
                {item.url ? (
                  <Link href={item.url}>
                    <item.icon />
...
export default function DashboardSidebar({}: Props) {
  ...
  const otherMenuItems = omi(clerk);
    ...
    <Sidebar collapsible="icon">
      <SidebarHeader className="flex flex-col gap-4 pt-4">
        <div className="flex items-center gap-2 pl-1 group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:pl-0">
          ...
          />
          <span className="group-data-[collapsible=icon]:hidden font-semibold text-xl tracking-normal text-foreground uppercase">
            Resonance
          </span>
            ...
            <OrganizationSwitcher
              ...
              appearance={{
                elements: {
                  ...
                  organizationPreviewMainIdentifier: "text-[16px]!",
